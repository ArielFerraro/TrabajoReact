import React from "react";
import {useState, useEffect} from "react";
import ItemDetail from "./ItemDetail";
import { useParams } from "react-router-dom";


const objetoDetalle = [
    {
    nombre: "Remera Rey Leon",
    imagen: "http://d3ugyf2ht6aenh.cloudfront.net/stores/440/495/products/remeras-leon-con-corona1-f3fef1574aa2aef7c915987989224604-640-0.png",
    id: 0,
    descripcion: "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Officiis commodi ea consequuntur necessitatibus itaque totam perspiciatis debitis nulla illo incidunt, magni asperiores enim eligendi quaerat magnam cupiditate rerum ex atque.",
    precio: "$2000",
},
    {
    nombre: "Remera Les Traigo Paz",
    imagen: "https://steamp.com.ar/wp-content/uploads/2022/02/les-traigo-paz-burns-sin-fondo-324x324.png",
    id: 1,
    precio: "$2500", 
},
    {
    nombre: "Remera Dalmata",
    imagen: "http://d3ugyf2ht6aenh.cloudfront.net/stores/440/495/products/remeras-dalmata-adelante1-cb0142e1cd69c2260515981905432670-640-0.png",
    id: 2,
    precio: "$3000",

    
},
    {
    nombre: "Remera C",
    imagen: "https://assets.stickpng.com/thumbs/580b57fbd9996e24bc43bf79.png",
    id: 3,
    precio: "$2000",

    
},
    {
    nombre: "Remera Rallada",
    imagen: "https://www.distritomoda.com.ar/sites/default/files/styles/producto_interior/public/imagenes/2126_negro_y_blanco_sin_fondo_1629462403.png?itok=350drpui",
    id: 4,
    precio: "$3500",  
},
    {
    nombre: "Remera Rey Leon",
    imagen: "https://w7.pngwing.com/pngs/3/819/png-transparent-printed-t-shirt-polo-shirt-clothing-t-shirt-tshirt-fashion-active-shirt-thumbnail.png",
    id: 5,
    precio: "$1500",
},

    {
    nombre: "Remera Blanca",
    imagen: "https://tienda.guantexindustrial.com.ar/712-home_default/remera-algodon-jersey-blanco-talle-l.jpg",
    id: 6,
    precio: "$3000",
},
    {
    nombre: "Remera Talento De Barrio",
    imagen: "https://i.servimg.com/u/f66/14/56/33/56/tm/remera13.png",
    id: 7,
    precio: "$4000",  
},
    {
    nombre: "Remera Azul",
    imagen: "https://tienda.crearteweb.com.ar/wp-content/uploads/2021/03/4.png",
    id: 8,
    precio: "$2500",  
},
    {
    nombre: "Remera Rey Leon",
    imagen: "https://icon2.cleanpng.com/20180409/dqe/kisspng-t-shirt-amazon-com-raglan-sleeve-clothing-camisa-5acaf00117a133.7572112415232491530968.jpg",
    id: 9,
    precio: "$2000",
},
    {
    nombre: "Remera Rey Leon",
    imagen: "http://d3ugyf2ht6aenh.cloudfront.net/stores/440/495/products/remeras-leon-con-corona1-f3fef1574aa2aef7c915987989224604-640-0.png",
    id: 10,
    precio: "$2000",  
},
]


const promesa = new Promise((res, rej) => {
    setTimeout(() => {
    res(objetoDetalle);
    }, 2000);
});



const ItemDetailContainer = () => {
    
    const [mostrarDetalle, setMostrarDetalle] = useState (null);
    const [loading, setLoading] = useState (false);
    
    const {itemid} = useParams ()

    

    useEffect(() =>{
        setLoading(true);
        promesa.then ((res) => {
            setLoading(false);
            setMostrarDetalle(res); 
            
        });
    },[itemid]);

    if (loading) {
        return (
            <>
            <h1>Cargando...Espere unos segundos</h1>
            </>
        );

    }
        return ( 
        <>
        <ItemDetail detalle={objetoDetalle}/>
        </>
        )
}


export default ItemDetailContainer